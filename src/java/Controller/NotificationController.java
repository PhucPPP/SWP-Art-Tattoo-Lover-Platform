...
public class NotificationController extends HttpServlet {
    ...
    private static final String ERROR = "notification.jsp";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (us != null) {
                List<NotificationDTO> listNotice = noticeDao.getNoticeList(userID);
                if (listNotice.size() > 0) {
                    request.setAttribute("NOTICE_LIST", listNotice);
                    url = SUCCESS;
                } else {
                    request.setAttribute("ERROR_MESSAGE", "Không có thông báo");
                    url = ERROR;
                }
            }
        } catch (Exception e) {
            log("Error ar NotificationController: " + e.toString());
        } finally {
